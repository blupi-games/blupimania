...
typedef struct
{
	short		idata;			/* permet de rgnrer le pointeur pdata (*) */
	short		*pdata;			/* pointeur au data (fixe) */
	short		offset;			/* offset  partir de pdata */
	short		size;			/* taille du data (1..3) */
	short		loop;			/* nb de boucles  effectuer */
	short		index;			/* index */
	short		indexmax;		/* index maximal */
}
Auto;

/*	(*)	Chaque fois que pdata est initialis, idata l'est aussi, afin de permettre
		de rgnrer pdata avec une procdure "Conv*ToTab*". Ceci est ncessaire
		lorsque la structure de l'automate est sauve puis reprise sur disque !
 */



/* --------------------------- */
/* Descripteur d'un toto anim */
/* --------------------------- */

#define OFFZTOTO	6		/* dcalage en Z pour toto */
...
typedef struct
{
	Status		status;				/* tat gnral */
	short		joueur;				/* 1 si c'est le joueur */
	short		force;				/* force (60..0) */
	short		vitesse;			/* vitesse (-2..+2) */
	short		vision;				/* vision (0..1) */
	short		mechant;			/* mchant (0..1) */
	short		tank;				/* tank (0..4) */
	short		magic;				/* passe-muraille (100..0) */
	short		invincible;			/* invincible (100..0) */
	short		cles;				/* cls trouves */
	Pt			poscel;				/* position cellule dans le monde */
	Pt			poscela;			/* position cellule d'arrive */
	Pt			poscelb;			/* position cellule d'arrive lointaine (si saute) */
	Action		action;				/* action en cours */
	Pt			lastpos;			/* dernire cellule atteinte */
	Pt			lastobjet;			/* coordonnes du dernier objet utilis */
	short		cntnotuse;			/* dure pour ne pas utiliser */
	short		lastrot;			/* dernier sens de rotation utilis */
	Pt			rstdecorcel;		/* cellule du dcor  restaurer */
	short		rstdecoricon;		/* icne du dcor  restaurer */
...
/* ------------------------------------- */
/* Descripteur d'un objet du dcor anim */
/* ------------------------------------- */
...
typedef struct
{
	Pt			cel;			/* cellule o est l'ascenseur */
	short		freq;			/* frquence des dparts */
	short		count;			/* dcompteur */
}
DepartMove;
...
static TotoMove		toto[MAXTOTO];				/* totos anims */
static TotoMove		back[MAXTOTO];				/* tats prcdents */
static DepartMove	depart[MAXDEPART];			/* ascenseurs */
static ObjetMove	objet[MAXOBJET];			/* objets du dcor anims */
static short		nbtoto;						/* nb de toto en mouvement simultanment */
static short		nbout;						/* nb de toto  sortir */
static short		nbin;						/* nb de toto  rentrer */
static short		perdu;						/* 1 -> c'est perdu */
static short		redraw;						/* 1 -> redessine tout */
static short		lastdetect;					/* dernier toto dtect pour les infos */
static short		lasttelecom;				/* dernire action de la tlcommande */
static Pt			celcap1, celcap2;			/* cellule de cap pour le toto du joueur */
static short		vitessepousse;				/* vitesse  laquelle pousser une caisse */
static Action		caisseodir;					/* boule qui roule: direction */
static Action		caisseoddir;				/* boule qui roule: dernire direction */
static Pt			caisseocel;					/* boule qui roule: dpart */
static short		gendecor;					/* numro de gnration du dcor */
static short		genstarttank;				/* numro de gnration dernier dpart tank */



typedef struct
{
	short		nbtoto;						/* nb de toto en mouvement simultanment */
	short		nbout;						/* nb de toto  sortir */
	short		nbin;						/* nb de toto  rentrer */
	short		lastdetect;					/* dernier toto dtect pour les infos */
	Pt			celcap1, celcap2;			/* cellule de cap pour le toto du joueur */
	Action		caisseodir;					/* boule qui roule: direction */
	Action		caisseoddir;				/* boule qui roule: dernire direction */
	Pt			caisseocel;					/* boule qui roule: dpart */
...
void AutoInit (Auto *p, short idata, short *pdata, short size)
{
	p->idata  = idata;
	p->pdata  = pdata;
	p->offset = 0;
...
/*
	Cette procdure est appels chaque fois qu'une cellule du dcor
	a t modifie.
 */
...
/*
	Retourne 1 si le toto est sur la cellule de dpart.
	Retourne 2 si le toto est sur la cellule d'arrive.
...
short MoveGetCel (Pt cel)
{
	...
	for ( i=0 ; i<MAXTOTO ; i++ )
	{
		...
		if ( toto[i].poscel.x  == cel.x && toto[i].poscel.y  == cel.y )  return 1;
...
/*
	Dtermine l'obstacle ventuel plac sur une cellule.
	Il peut s'agit d'un objet du dcor ou d'un autre toto !
	Retourne 0 s'il n'y a pas d'obstacle.
		toto = 1	->	tient compte des toto
		toto = 0	->	ne tient compte que du dcor
 */
...
/*
	Donne l'orientation correspondant  une action.
	Retourne une valeur ACMARCHE?? selon le sens.
 */

Action GetOrientation (Action action)
{
	switch( action )
	{
		case AC_STOP_E:
...
/*
	Donne le sens de la rotation correspondant  une action.
...
short GetRotation (Action action)
{
	switch( action )
	{
		case AC_TOURNE_NE:
...
/*
	Bouge si ncessaire un objet anim du dcor.
 */

void ObjetNextOne (short i)
{
	short		result[1];
	short		icon;
	...
	else
	{
		...
		if ( icon != 0 )
		{
			DecorModif(objet[i].cel, icon);		/* modifie le dcor */
		}
	}
}
...
/*
	Bouge si ncessaire les objets anims du dcor.
 */
...
/*
	Gnre un mouvement quelconque selon une table.
 */

void TableMovie(short i, Action action, short *ptable)
{
...
/*
	Initialise un mouvement linaire rgulier, dans le plan (z=0).
		nbmove	->	nb de pas  effectuer
		lg		->	dplacement  effectuer (+/- dx;dy)
 */

void InitMovie(short i, short nbmove, Pt lg)
{
...
/*
	Calcule un mouvement linaire rgulier, dans le plan (z=0).
	Retourne 1 (true) si le mouvement est termin.
 */
...
short CalcMovie(short i, short result[])
{
	...
	result[1] = new.y - last.y;		/* dplacement selon y */
	result[2] = 0;					/* dplacement selon z */
...
/*
	Donne le type de marche  utiliser pour un toto donn.
		0 -> marche rapide en 4 mouvements (1,2,1,3)
		1 -> marche plus jolie
 */

short GetTypeMarche (short i)
{
	if ( toto[i].force   >= 5 &&
		 toto[i].vision  == 1 &&
		 toto[i].mechant == 0 &&
...
void NewAction(short i, Action action, short posz)
{
	...
	if ( action >= AC_MARCHE_E &&				/* toto doit marcher ? */
		 ...
		 ((toto[i].action >= AC_STOP_E &&		/* toto est stopp ? */
		  ...
		  (toto[i].action >= AC_VISION_E &&		/* toto a reu des lunettes ? */
		   toto[i].action <= AC_VISION_S) ||
		  (toto[i].action >= AC_REPOS_E &&		/* toto s'est repos ? */
		   toto[i].action <= AC_REPOS_S) ||
		  (toto[i].action >= AC_REFLEXION_E &&	/* toto s'est gratt la tte ? */
		   toto[i].action <= AC_REFLEXION_S) ||
		  (toto[i].action >= AC_YOYO_E &&		/* toto a jou au yoyo ? */
		   toto[i].action <= AC_YOYO_S) ||
		  (toto[i].action >= AC_HAUSSE_E &&		/* toto a hauss les paules ? */
...
/*
	Retourne l'obstacle prsent dans une cellule, comme GetObstacle.
...
/*
	...
	(mur du dcor, autre toto, etc.).
	Essaye de le tourner dans le mme sens que le prcdent utilis !
 */

void TourneAction(short i)
{
	...
	if ( toto[i].vision == 0 )		/* est-ce que toto est aveugle ? */
	{
		do
		{
			...
			n ++;
			if ( toto[i].lastrot == 0 )  break;
			if ( toto[i].lastrot == GetRotation(nextaction) )  break;
			if ( n > 10 )  break;
		}
		while ( 1 );
	}
	else							/* si toto n'est pas aveugle */
	{
		...
		switch( orientation )
		{
			case AC_MARCHE_E:
				...
				nextaction = -1;
				if ( (left.x != toto[i].lastpos.x ||
					  left.y != toto[i].lastpos.y ) &&
					 leftob == 0 )			nextaction = AC_TOURNE_EN;
				if ( (right.x != toto[i].lastpos.x ||
			...
			case AC_MARCHE_O:
				...
				if ( (left.x != toto[i].lastpos.x ||
					  left.y != toto[i].lastpos.y ) &&
					 leftob == 0 )			nextaction = AC_TOURNE_OS;
				if ( (right.x != toto[i].lastpos.x ||
					  right.y != toto[i].lastpos.y ) &&
			...
			case AC_MARCHE_S:
				...
				if ( (left.x != toto[i].lastpos.x ||
					  left.y != toto[i].lastpos.y ) &&
					 leftob == 0 )			nextaction = AC_TOURNE_SE;
				if ( (right.x != toto[i].lastpos.x ||
					  right.y != toto[i].lastpos.y ) &&
			...
			case AC_MARCHE_N:
				...
				if ( (left.x != toto[i].lastpos.x ||
					  left.y != toto[i].lastpos.y ) &&
					 leftob == 0 )			nextaction = AC_TOURNE_NO;
				if ( (right.x != toto[i].lastpos.x ||
					  right.y != toto[i].lastpos.y ) &&
...
/*
	Fait pter toutes les bombes correspondant  un dtonateur.
 */

void BombeBaoum (short detonateur)
{
	short		bombe;
	Pt			cel;
	...
	for ( cel.y=0 ; cel.y<MAXCELY ; cel.y++ )
	{
		for ( cel.x=0 ; cel.x<MAXCELX ; cel.x++ )
		{
			...
			{
				if ( bombe == ICO_BOMBE_A )  ObjetPut(cel, OB_BOMBEA);
				if ( bombe == ICO_BOMBE_B )  ObjetPut(cel, OB_BOMBEB);
				if ( bombe == ICO_BOMBE_C )  ObjetPut(cel, OB_BOMBEC);
			}
		}
	}
}
...
/*
	Regarde s'il est possible de sauter par dessus un objet du dcor
	pas trop haut.
	Retourne 0 si c'est possible.
 */

short SauteAction(short i, Pt celsaut)
{
	short		icon, obstacle;
	Pt			celarr;
...
/*
	Vrifie s'il est possible de pousser une caisse.
	Dmarre ventuellement les actions associes.
...
short IfPousseCaisse (Pt celarr, short caisse, Action orientation)
{
	...
	if ( icon == 0 ||
		 ...
		 (icon >= ICO_ACCEL_S &&
		  icon <= ICO_ACCEL_O) ||
		 ((icon == ICO_VITRE+4 || icon == ICO_VITRE+5) &&
		  caisse == ICO_CAISSEO ) )
	{
		if ( icon == ICO_TROU )
		{
			DecorModif(celarr, ICO_TROUBOUCHE);		/* met un trou bouch */
		}
		if ( icon == ICO_SENSUNI_O && orientation == AC_MARCHE_E )
		{
			if ( MoveGetCel(celarr) == 0 )
				ObjetPut(celarr, OB_SENSUNIO);		/* monte puis redescend la flche */
			return 0;								/* impossible */
		}
		if ( icon == ICO_SENSUNI_E && orientation == AC_MARCHE_O )
		{
			if ( MoveGetCel(celarr) == 0 )
				ObjetPut(celarr, OB_SENSUNIE);		/* monte puis redescend la flche */
			return 0;								/* impossible */
		}
		if ( icon == ICO_SENSUNI_N && orientation == AC_MARCHE_S )
		{
			if ( MoveGetCel(celarr) == 0 )
				ObjetPut(celarr, OB_SENSUNIN);		/* monte puis redescend la flche */
			return 0;								/* impossible */
		}
		if ( icon == ICO_SENSUNI_S && orientation == AC_MARCHE_N )
		{
			if ( MoveGetCel(celarr) == 0 )
				ObjetPut(celarr, OB_SENSUNIS);		/* monte puis redescend la flche */
			return 0;								/* impossible */
		}
		if ( icon == ICO_VITRE+4 )
		{
...
short SearchTotoForTank (Pt cel)
{
	...
	for ( i=0 ; i<MAXTOTO ; i++ )
	{
		if ( toto[i].status == STVIDE ||
			 ...
			 (toto[i].action >= AC_ARRIVEE_E &&
			  toto[i].action <= AC_ARRIVEE_S) ||
			 toto[i].action == AC_ARRIVEE_M ||
...
/*
	Effectue une action spciale, en fonction de l'obstacle rencontr.
	Retourne 1 si une action est entreprise.
 */

short SpecAction(short i, short obstacle, Pt testcel)
{
	...
	if ( toto[i].tank != 0 )
	{
		if ( obstacle == 1 || obstacle == 2 ||		/* tank arrive sur un toto ? */
			 (obstacle >= ICO_SENSUNI_S &&
			  obstacle <= ICO_SENSUNI_O) )
		{
		 ...
		 MoveGetCel(testcel) == 0 )
	{
		if ( orientation == AC_MARCHE_N )
		{
			if ( toto[i].joueur )  celcap1.x = -1;
			TourneAction(i);
			ObjetPut(testcel, OB_SENSUNIS);			/* monte puis redescend la flche */
		}
		else
		{
			NewAction(i, orientation, 0);			/* passe sur la flche */
		}
		return 1;
	}
		 ...
		 MoveGetCel(testcel) == 0 )
	{
		if ( orientation == AC_MARCHE_S )
		{
			if ( toto[i].joueur )  celcap1.x = -1;
			TourneAction(i);
			ObjetPut(testcel, OB_SENSUNIN);			/* monte puis redescend la flche */
		}
		else
		{
			NewAction(i, orientation, 0);			/* passe sur la flche */
		 ...
		 MoveGetCel(testcel) == 0 )
	{
		if ( orientation == AC_MARCHE_O )
		{
			if ( toto[i].joueur )  celcap1.x = -1;
			TourneAction(i);
			ObjetPut(testcel, OB_SENSUNIE);			/* monte puis redescend la flche */
		}
		else
		{
			NewAction(i, orientation, 0);			/* passe sur la flche */
		 ...
		 MoveGetCel(testcel) == 0 )
	{
		if ( orientation == AC_MARCHE_E )
		{
			if ( toto[i].joueur )  celcap1.x = -1;
			TourneAction(i);
			ObjetPut(testcel, OB_SENSUNIO);			/* monte puis redescend la flche */
		}
		else
		{
			NewAction(i, orientation, 0);			/* passe sur la flche */
	...
	if ( obstacle == ICO_ACCEL_S &&					/* acclrateur vers le sud ? */
		 MoveGetCel(testcel) == 0 )
	{
		if ( orientation == AC_MARCHE_S )  toto[i].vitesse = +2;
		if ( orientation == AC_MARCHE_N )  toto[i].vitesse = -2;
	...
	if ( obstacle == ICO_ACCEL_E &&					/* acclrateur vers l'est ? */
		 MoveGetCel(testcel) == 0 )
	{
		if ( orientation == AC_MARCHE_E )  toto[i].vitesse = +2;
		if ( orientation == AC_MARCHE_O )  toto[i].vitesse = -2;
		NewAction(i, orientation, 0);
		return 1;
	}
	...
	if ( obstacle == ICO_ARRIVEE )					/* est-on sur l'arrive ? */
	{
		...
		if ( toto[i].mechant == 0 )
		{
			...
			DecorModif(testcel, ICO_ARRIVEEPRIS);		/* le ballon est pris (si autre toto) */
		}
		else
		{
		 ...
		 toto[i].mechant == 0 )
	{
		...
		DecorModif(testcel, ICO_TOURTE+1);			/* enlve la tourte sur la table */
		 ...
		 toto[i].mechant == 0 )
	{
		...
		DecorModif(testcel, ICO_CLE_VIDE);			/* dcroche la cl */
		 ...
		 toto[i].mechant == 0 )
	{
		if ( toto[i].joueur )  celcap1.x = -1;
		toto[i].cles &= ~(1<<(obstacle-ICO_PORTEF_EO));
		DecorModif(testcel, obstacle+(ICO_PORTEO_EO-ICO_PORTEF_EO));	/* ouvre la porte */
		 ...
		 toto[i].mechant == 0 )
	{
		if ( toto[i].joueur )  celcap1.x = -1;
		toto[i].cles &= ~(1<<(obstacle-ICO_PORTEF_NS));
		DecorModif(testcel, obstacle+(ICO_PORTEO_NS-ICO_PORTEF_NS));	/* ouvre la porte */
		NewAction(i, orientation+AC_PORTE_E-AC_MARCHE_E, 0);
		return 1;
	}
	...
	if ( obstacle == ICO_GLISSE )					/* glisse sur une peau de banane ? */
	{
		...
		if ( toto[i].joueur )  celcap1.x = -1;
		toto[i].mechant = 1;						/* devient mchant */
		 ...
		 toto[i].vision  == 1 )
	{
		...
		{
			if ( obstacle == ICO_CAISSE  )  vitessepousse = 14;
			if ( obstacle == ICO_CAISSEV )  vitessepousse = 8;
			if ( obstacle == ICO_CAISSEO )  vitessepousse = 10;
			if ( obstacle == ICO_CAISSEG )  vitessepousse = 10;
			...
			if ( IfPousseCaisse(cel, obstacle, orientation) )
			{
				...
				{
					rang = MoveInit(testcel, orientation+AC_CAISSEV_E-AC_MARCHE_E, 0);
				}
				if ( obstacle == ICO_CAISSEO )
				{
					rang = MoveInit(testcel, orientation+AC_CAISSEO_E-AC_MARCHE_E, 0);
				}
				if ( obstacle == ICO_CAISSEG )
				{
					rang = MoveInit(testcel, orientation+AC_CAISSEG_E-AC_MARCHE_E, 0);
				}
				if ( rang >= 0 )
				{
					...
					if ( obstacle == ICO_CAISSEO ||
						 obstacle == ICO_CAISSEG )
					{
						toto[rang].energie = 2;		/* nergie d'une boule pousse */
					}
					redraw = 1;						/* faudra tout redessiner */
				}
				return 1;
		 ...
		 toto[i].mechant == 1 )
	{
		...
		if ( obstacle == ICO_DETONATEUR_A )  ObjetPut(testcel, OB_DETONATEURA);
		if ( obstacle == ICO_DETONATEUR_B )  ObjetPut(testcel, OB_DETONATEURB);
		if ( obstacle == ICO_DETONATEUR_C )  ObjetPut(testcel, OB_DETONATEURC);
		BombeBaoum(obstacle);
		return 1;
	}
...
/*
	...
	Si back = 1, cela signifie qu'on teste une cellule situe derrire
...
/*
	Regarde s'il y a une cellule intressante dans une direction donne.
	Si oui, retourne 1 (true).
 */

short IfLineAction (short i, Pt cel, Action direction, short back)
{
	...
	switch	( direction )
	{
		case AC_MARCHE_E:  dir.x ++;  break;
		case AC_MARCHE_O:  dir.x --;  break;
		case AC_MARCHE_S:  dir.y ++;  break;
	...
	for ( nb=0 ; nb<10 ; nb++ )
	{
			 ...
			 toto[i].tank    == 0 )
		{
				  ...
				  icon == ICO_LUNETTES )			/* lunettes ? */
			{
				...
				if ( obstacle == 0 ||				/* libre aprs l'obstacle bas ? */
					 obstacle == ICO_AIMANT )
				{
					pos.x = cel.x - dir.x;
					pos.y = cel.y - dir.y;
					obstacle = GetObstacle(pos, 0);
					if ( obstacle == 0 ||			/* libre avant l'obstacle bas ? */
						 obstacle == ICO_AIMANT )
					{
						continue;
					}
		...
		if ( icon == ICO_GLISSE ||					/* peau de banane ? */
			 ...
			 (icon >= ICO_ACCEL_S &&
			  icon <= ICO_ACCEL_O) )				/* le regard passe par-dessus un acclrateur */
		{
			continue;
		}
...
/*
	Teste si toto dpasse un mur en coin, et s'il faut ventuellement
	tourner plutt que de continuer toujours tout droit.
	Retourne 1 (true) si toto dcide de tourner.
 */

short TurnCoin (short i, Action *pnextaction)
{
	...
	if ( GetMur(bleft, 0) >= ICO_BLOQUE &&				/* mur  l'arrire gauche ? */
		 ...
		 (left.x != toto[i].lastpos.x ||
		  left.y != toto[i].lastpos.y ) )				/* ne vient-on pas dj de l ? */
	{
		*pnextaction = acleft;
		return 1;
	}
	...
	if ( GetMur(bright, 0) >= ICO_BLOQUE &&				/* mur  l'arrire droite ? */
		 ...
		 (right.x != toto[i].lastpos.x ||
		  right.y != toto[i].lastpos.y ) )				/* ne vient-on pas dj de l ? */
	{
		*pnextaction = acright;
		return 1;
	}
...
/*
	Si toto est dot de la vision, regarde s'il n'y a pas mieux  faire
	que de marcher tout droit.
	Retourne 1 (true) si y'a qq chose de mieux  faire.
 */
...
/*
	Fait ventuellement entendre un son d'ambiance selon
...
/*
	Cherche une action pour le toto pilot par le joueur.
 */

void JoueurAction (short i, char event, Action orientation, Pt testcel)
{
	short		obstacle;
	short		retry = 0;
	KeyStatus	keystatus;
...
/*
	Vrifie si le trajet sur une ligne droite est libre d'obstacles.
	Si oui, retourne 1 (true).
	Ne teste pas la cellule d'arrive !
 */

short IfFreeLine (Pt depart, Pt arrivee)
{
	Pt		dir;
	short	obstacle;
...
/*
	Retourne le numro du toto du joueur.
...
/*
	Cherche un nouveau cap  atteindre pour le toto du joueur en fonction
	de la cellule clique dans le dcor.
 */

void JoueurCap (char event, Pt pmouse)
{
	Pt			cel, inter;
	short		i;
...
/*
	...
	Cette procdure est appele chaque fois qu'une action quelconque
	est termine, pour dterminer ce que toto fera ensuite.
 */

void NextAction(char event, short i)
{
	Pt			testcel;
	Pt			p;
	Action		nextaction;
	...
	{
		p = toto[i].lastobjet;
		DecorModif(toto[i].rstdecorcel, toto[i].rstdecoricon);	/* restaure le dcor */
		toto[i].rstdecoricon = 0;
		toto[i].lastobjet = p;
	}
	...
	if ( toto[i].force > 0 &&
		 ...
		 (toto[i].action < AC_DORT_E ||
		  toto[i].action > AC_DORT_S) &&
		 (toto[i].action < AC_REFLEXION_E ||
		  toto[i].action > AC_REFLEXION_S) &&
		 (toto[i].action < AC_YOYO_E ||
		  toto[i].action > AC_YOYO_S) &&
	...
	if ( (toto[i].poscela.x != toto[i].poscel.x ||
		 ...
		 toto[i].icon != ICO_CAISSEV &&
		 toto[i].icon != ICO_CAISSEO &&
		 toto[i].icon != ICO_CAISSEG &&
		 ...
		 toto[i].action <= AC_ARRIVEE_S )
	{
		...
		MoveInit(testcel, AC_BALLON_M, -LYICO);		/* dpart du ballon */
	...
	if ( (toto[i].action >= AC_TOMBE_E &&
		 ...
		 (toto[i].action >= AC_EXPLOSE_E &&
		  toto[i].action <= AC_EXPLOSE_S) ||
		 toto[i].action == AC_TANK )
	{
		toto[i].status = STVIDE;					/* ce toto n'existe plus */
		nbtoto --;
		if ( toto[i].mechant == 0 &&
			 toto[i].tank    == 0 )  nbout ++;		/* il pourra ressortir */
		if ( toto[i].joueur )
		{
			toto[i].joueur = 0;
			perdu = 1;
		...
		if ( toto[i].tank == 3 )
		{
			DecorModif(toto[i].poscel, ICO_TANKBAS);		/* met un sol bouch par le tank */
			DecorPutInitCel(toto[i].poscel, ICO_TANKBAS);	/* modification permanante */
		}
		return;
	}
		 ...
		 toto[i].action <= AC_CAISSEOD_S )
	{
		...
		{
			if ( toto[i].energie == 2 )					/* nergie d'une boule pousse ? */
			{
		 ...
		 toto[i].action <= AC_CAISSEG_S )
	{
		...
		else
		{
			if ( toto[i].energie == 2 )					/* nergie d'une boule pousse ? */
			{
				if ( orientation == AC_MARCHE_E )  testcel.x ++;
				if ( orientation == AC_MARCHE_O )  testcel.x --;
				if ( orientation == AC_MARCHE_S )  testcel.y ++;
				if ( orientation == AC_MARCHE_N )  testcel.y --;
	...
	if ( toto[i].action == AC_CAISSE_T )
	{
		...
		DecorPutInitCel(toto[i].poscel, ICO_CAISSEBAS);	/* modification permanante */
		toto[i].status = STVIDE;						/* ce toto n'existe plus */
		nbtoto --;
		return;
	}
	...
	if ( toto[i].action == AC_CAISSEV_T )
	{
		...
		DecorPutInitCel(toto[i].poscel, ICO_CAISSEVBAS);/* modification permanante */
		toto[i].status = STVIDE;						/* ce toto n'existe plus */
		nbtoto --;
		return;
	}
	...
	if ( toto[i].action == AC_CAISSEO_T )
	{
		...
		DecorPutInitCel(toto[i].poscel, ICO_CAISSEOBAS);/* modification permanante */
		toto[i].status = STVIDE;						/* ce toto n'existe plus */
		nbtoto --;
		return;
	}
	...
	if ( toto[i].action == AC_CAISSEG_T )
	{
		...
		DecorPutInitCel(toto[i].poscel, ICO_CAISSEGBAS);/* modification permanante */
		toto[i].status = STVIDE;						/* ce toto n'existe plus */
		nbtoto --;
		return;
	}
	...
	{
		NewAction(i, nextaction, 0);		/* si c'est possible -> dmarre la nouvelle action */
	}
	else
	{
		if ( !SpecAction(i, obstacle, testcel) )	/* effectue une action spciale */
		{
			if ( toto[i].mechant == 0 &&
				 toto[i].tank == 0 )
			{
...
/*
	Fait faire un pas en arrire au toto plac sur une cellule donne.
 */
...
void MoveBack (Pt cel)
{
	...
	for ( i=0 ; i<MAXTOTO ; i++ )
	{
			 ...
			 (toto[i].poscelb.x == cel.x && toto[i].poscelb.y == cel.y) )
		{
			...
			{
				toto[i] = back[i];		/* reprend l'tat prcdent */
			}
		}
	}
}
...
/*
	Gnre une table trie contenant les indices dans toto[] avec
	en premier les totos placs les plus au fond, et en dernier ceux
	qui sont tout devant. Ceci est ncessaire, pour dessiner avec
	IconDrawPut  partir du fond jusque devant.
	Trie avec un "bubble-sort" pas fameux du tout, mais cela ne gne
	pas trop, vu le petit nombre de toto  trier (<10) !
 */

void TrieToto (char *ptable)
{
	short		i, ii, j;
	short		v, vv;
	short		job;
	...
	while ( job != 0 );		/* rpre tant que qq a t fait */
}
...
/*
	Dtecte le toto vis par la souris.
	Retourne -1 si y'en a aucun !
 */

short DetectToto (Pt pmouse, Pt ovisu, char ordre[])
{
	short		i, j;
	Pt			pos;
		 ...
		 pmouse.y >= 0 && pmouse.y <= DIMYDRAW )
	{
		for ( j=MAXTOTO-1 ; j>=0 ; j-- )	/* du plus en avant au plus en arrire */
		{
			...
			if ( toto[i].status == STVIDE ||
				 ...
				 (toto[i].action >= AC_CAISSE_E &&
				  toto[i].action <= AC_CAISSE_S) ||
				 (toto[i].action >= AC_CAISSEV_E &&
				  toto[i].action <= AC_CAISSEV_S) ||
				 (toto[i].action >= AC_CAISSEG_E &&
				  toto[i].action <= AC_CAISSEG_S) ||
			...
			if ( pmouse.x >= pos.x+15       &&
				 ...
				 pmouse.y >= pos.y+15       &&
				 pmouse.y <= pos.y+LYICO-5  )
			{
				lastdetect = i;
				return i;					/* retourne le toto trouv */
			}
		}
	}
...
/*
	...
	est interruptible, suite  un vnement prioritaire.
 */
...
/*
	Dessine un toto anim.
 */

void DrawOne (short i, Pt ovisu)
{
	short		btransp;
	Reg			r;
	Pt			pos;
...
/*
	Fait ventuellement partir les tanks stopps si y'a un toto qui s'pointe.
 */
	...
	for ( cel.y=0 ; cel.y<=MAXCELY ; cel.y++ )
	{
		for ( cel.x=0 ; cel.x<=MAXCELX ; cel.x++ )
		{
			...
			if ( icon == ICO_TANK_X  ||		/* est-ce un tank immobile (dans le dcor) ? */
				 ...
				 icon == ICO_TANK_NS )
			{
				i = MoveInit(cel, AC_STOP_E, 0);					/* prpare le tank */
				if ( i >= 0 )
				{
					...
					else
					{
						...
						if ( icon == ICO_TANK_X )  toto[i].tank = 2;
						else                       toto[i].tank = 3;
...
/*
	Avance les totos anims d'un "pas".
	Retourne 1 si le monde est termin (tous les toto rentrs).
	Retourne 2 si le monde est termin (perdu).
 */

short MoveNext (char event, Pt pmouse)
{
	short		i, j;
	short		try;
	Pt			ovisu;
	...
	for ( j=0 ; j<MAXTOTO ; j++ )
	{
		...
		{
			NextAction(event, i);					/* oui -> affecte une autre action  toto */
			try ++;
			goto next;
		}
		...
		{
			orientation = GetOrientation(toto[i].action);
			if ( orientation == AC_MARCHE_E )  toto[i].icon = ICO_TANK_E;
			if ( orientation == AC_MARCHE_N )  toto[i].icon = ICO_TANK_N;
			if ( orientation == AC_MARCHE_O )  toto[i].icon = ICO_TANK_O;
			if ( orientation == AC_MARCHE_S )  toto[i].icon = ICO_TANK_S;
			if ( toto[i].tank == 2 )           toto[i].icon = ICO_TANK_X;
			if ( toto[i].tank == 3 )
			{
				if ( orientation == AC_MARCHE_E ||
					 orientation == AC_MARCHE_O )  toto[i].icon = ICO_TANK_EO;
				if ( orientation == AC_MARCHE_N ||
	...
	{
		celarr = caisseocel;
		if ( caisseoddir == AC_CAISSEO_E )  celarr.x ++;
		if ( caisseoddir == AC_CAISSEO_O )  celarr.x --;
		if ( caisseoddir == AC_CAISSEO_S )  celarr.y ++;
		if ( caisseoddir == AC_CAISSEO_N )  celarr.y --;
		if ( IfPousseCaisse(celarr, ICO_CAISSEO, caisseoddir-AC_CAISSEO_E+AC_MARCHE_E) )
		{
			DecorModif(caisseocel, DecorGetInitCel(caisseocel));	/* enlve la boule fixe */
			i = MoveInit(caisseocel, caisseoddir+AC_CAISSEOD_E-AC_CAISSEO_E, 0);
			if ( i >= 0 )
			{
				toto[i].icon = ICO_CAISSEO;
				toto[i].energie = 1;			/* nergie d'une boule sur son lan */
				redraw = 1;						/* faudra tout redessiner */
			}
		}
	}
...
/*
	Construit devant le toto du joueur, selon l'outil actionn.
...
/*
	Dcale le dcor si le toto du joueur dpasse de la partie visible.
 */
...
void MoveNewMonde (short freq)
{
	...
	for ( cel.y=0 ; cel.y<=MAXCELY ; cel.y++ )
	{
		...
		{
			icon = DecorGetCel(cel);
			if ( icon == ICO_DEPART &&		/* est-ce un ascenseur ? */
				 d < MAXDEPART )
			{
				nbdepart ++;
				depart[d].cel   = cel;
			...
			if ( icon == ICO_ARRIVEE )		/* est-ce une arrive (ballon) ? */
			{
				nbout ++;
			}
				                ...
				                g_typeedit == 0 )			/* n'est-on pas en train d'diter un monde ? */
			{
				...
				if ( nbjoueurs == 0 )
				{
					i = MoveInit(cel, AC_STOP_E, 0);		/* prpare le toto du joueur */
					if ( i >= 0 )
					{
...
/*
	...
	Les pointeurs contenus dans les automates (pdata) doivent tre
